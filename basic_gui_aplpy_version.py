...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 13 02:46:42 2022
...
#        self.target_list.rasie()
    ...
    def plot_table(self):
        try:
            ...
            fig = aplpy.FITSFigure(hdu, north=True, figure=self.canv.fig)
            fig.show_colorscale()
            fig.recenter(coord.ra.value, coord.dec.value, width=10/60, height=10/60)

            fov_width = 3.1/60 ## in arcsec
            delta_fov = 9.1/60 ## in arcsec
            coord_fov = SkyCoord(coord.ra+(0/3600)*u.deg, coord.dec+(0)*u.arcsec)                     

            fig.show_rectangles(coord.ra.value, coord.dec.value, fov_width, delta_fov,)   
            ...
            for i in range(5):
                ...
                fig.show_rectangles(coord_slit0.ra.value, coord_slit0.dec.value, slit0_width/3600, 9.1/5/60, edgecolor=c[i])   
